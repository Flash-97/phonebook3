...
public class PhoneDao {
	...
	@Autowired
	private SqlSession sqlSession; // new DataSource();
	 ...
	 * close(); return count; }
	 * 
	 * // 사람정보 public PersonVo getPerson(int personId) {
	 * 
	 * PersonVo personVo = null;
	 * 
	 * getConnection(); try { String query =
	 * "SELECT person_id, name, hp, company FROM person where person_id = ?"; // 쿼리문
	 * 문자열만들기, ? 주의
	 * 
	 * pstmt = conn.prepareStatement(query); // 쿼리로 만들기 pstmt.setInt(1, personId);
	 * 
	 * rs = pstmt.executeQuery();
	 * 
	 * // 4.결과처리 while (rs.next()) { String name = rs.getString("name"); String hp =
	 * rs.getString("hp"); String company = rs.getString("company");
	 * 
